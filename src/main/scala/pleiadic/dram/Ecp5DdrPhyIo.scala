...
class Ecp5DdrPads(config: DramConfig) extends Bundle {
...
/** Complete ECP5 x2 DDR3 I/O assembly around [[Ecp5DdrPhyOutput]]. */
class Ecp5DdrPhyIo(config: DramConfig, commandDelayInitialValue: Int = 0)
    extends RawModule {
  ...
  val io = IO(new Bundle {
    ...
    val pads = new Ecp5DdrPads(config)
  })

  private val init = withClockAndReset(io.initClock, io.reset) {
    Module(new Ecp5DdrPhyInit)
  }
  ...
  private def commandOutput(value: UInt): Bool = {
    val serializer = Module(new Ecp5OutputDdrX2)
    val delay = Module(new Ecp5CommandDelay(commandDelayInitialValue))
  ...
  io.pads.clockPositive := commandOutput(io.parallel.clock)
  ...
  for (byte <- 0 until padBytes) {
    ...
    val dqsBuffer = Module(new Ecp5DqsBuffer)
    val dqsOutput = Module(new Ecp5DqsOutputSerdes)
    ...
    dqsBuffer.io.dllDelay := init.io.delay
    ...
    dqsOutput.io.outputEnable := io.parallel.dqsOutputEnable
    dqsOutput.io.preamble := io.parallel.dqsPreamble
    dqsOutput.io.postamble := io.parallel.dqsPostamble
    ...
    val mask = Module(new Ecp5DataOutputSerdes)
    ...
    for (laneIndex <- 0 until 8) {
      ...
      val lane = Module(new Ecp5DataSerdesLane)
